...
\renewcommand{\Mytitle}{On Commonsense Domains within  \linebreak the Winograd Schema Challenge}
\usepackage{showexpl} 
\usepackage{array}
...
\begin{frame}
  \frametitle{Motivation}
	...
	\begin{itemize}
		\normalsize
		\item[S:] The trophy does not fit into the brown suitcase\\ because \alert{it} is too \only<1>{\alert{small}}\only<2,3>{\alert{large}}.%\alert{[small/large]}.
		\item[Q:] What is too  \only<1>{small}\only<2,3>{large}?
...
\onslide<1->	\begin{itemize}
	...
	\begin{itemize}
		\normalsize
		\item \textit{X causes/prevents/followed by Y}
	\end{itemize}
...
\onslide<1->\input{graph_weak.tex}
\begin{columns}
	\begin{column}{0.40\textwidth}
\onslide<2->	\item ``Who was weak?"
\onslide<2->	\input{graph_quest.tex} 
	\end{column}

	\begin{column}{0.30\textwidth}
\onslide<3->	\item ``weak y \textbf{prevents} y lifts"
\onslide<3->	\input{kn_type.tex} 
	\end{column}	
\end{columns}
%\item Knowledge: 
\end{itemize}
...
has_k(lifts,agent,y).		
\end{lstlisting}
\hspace{2.2cm}\centering {\alert{ans(q-1,he-9), ans(q-1,man-2)}}
\end{frame}
...
\end{lstlisting}
\hspace{2.2cm}\centering {\alert{ans(q-1,he-9), ans(q-1,son-7)}}
\end{frame}
...
\hspace*{-0.6cm}	\begin{tabular}{  l | l  }
...
\begin{frame}[fragile] 
\begin{itemize}
\onslide<1->	\item {\bf \large \textcolor{OrangeRed1}{Contributions}}\\
	  \begin{itemize}
		\normalsize
...
\onslide<2->    \item {\bf \large \textcolor{OrangeRed1}{Future Work}}\\
    \begin{itemize}
    	\normalsize
...
\onslide<3->\item {\bf \large \textcolor{OrangeRed1}{Thank you!}}
\end{itemize}
